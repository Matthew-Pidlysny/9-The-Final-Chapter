...
namespace AstraTechnica {
    ...
    bool PhoneIntegration::register_device(const std::string& device_name) {
        ...
        std::string response;
#ifdef NLOHMANN_JSON_FOUND
        response = send_to_server(registration_data.dump());
#else
        std::ostringstream oss;
        oss << "{&quot;device_id&quot;:&quot;" << device_id << "&quot;,&quot;device_name&quot;:&quot;" << device_name << "&quot;}";
        response = send_to_server(oss.str());
#endif
...
#else
                // Simple JSON-like parsing for fallback
                if (response.find("&quot;user_token&quot;") != std::string::npos) {
                    size_t start = response.find("&quot;user_token&quot;:&quot;") + 14;
                    size_t end = response.find("&quot;", start);
                    user_token = response.substr(start, end - start);
                    return true;
                }
#endif
            } catch (...) {
    ...
    void PhoneIntegration::generate_encryption_keys() {
        ...
        for (size_t i = 0; i < 32; ++i) {
            encryption_key[i] = static_cast<char>(dis(gen));
        }
#endif
    }
    ...
    void PhoneIntegration::process_notification_queue() {
        while (!notification_queue.empty()) {
            ...
            if (server_connected.load()) {
...
#else
                // Fallback JSON-like string construction
                std::ostringstream oss;
                oss << "{"
                    << "&quot;id&quot;:&quot;" << notification.id << "&quot;,"
                ...
                if (!notification.action_buttons.empty()) {
                    oss << ",&quot;actions&quot;:[";
                    for (size_t i = 0; i < notification.action_buttons.size(); ++i) {
                        if (i > 0) oss << ",";
                        oss << "&quot;" << notification.action_buttons[i] << "&quot;";
                    }
                    oss << "]";
                }
    ...
    bool PhoneIntegration::send_to_server(const std::string& data) {
        ...
        std::cout << "Server send (simulated): " << data.substr(0, 100) << "..." << std::endl;
        return true;
#endif
    }
